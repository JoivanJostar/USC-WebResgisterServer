...
int main() {
	map<string, map<string, string>> courseTable = LoadCourseInfo("ee.txt");
	cout << "The ServerEE is up and running using UDP on port " << UDP_serverEE_port << "." << endl;
	char on = 1;
	...
	if (bind(udp_sock, (struct sockaddr*)&udp_serverEE_addr, sizeof(struct sockaddr_in)) < 0) {
		perror("fail to bind");
		return -1;
	}
	...
	while (true) {
		...
		if (datas.size() == 1 && datas[0].second != "ALL") {
			...
			cout << "The ServerEE received a request from the Main Server about the " << toLowerCase(category) << " of " << courseCode << ". ";
			if (info.empty()) {
				memset(buf, 0, MAXBUFSIZE);

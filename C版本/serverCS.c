...
int main() {
	...
	int serverCS_udp_sock = socket(AF_INET, SOCK_DGRAM, 0);
	if (serverCS_udp_sock == -1) {
		perror("can not create socket");
		return -1;
	}

	if (bind(serverCS_udp_sock, (struct sockaddr*)&serverCS_udp_addr, sizeof(struct sockaddr_in)) < 0) {
	...
	while (1) {
		my_recv(UDP, g_recv_buffer, g_data, serverCS_udp_sock);
		...
		if (strcmp("ALL_CATEGORY", category) != 0) {
			printf("The ServerCS received a request from the Main Server about the %s of %s.", strlwr(category), course_code);
		}
		else {
		...
		my_send(UDP, g_send_buffer, g_data, 1, serverCS_udp_sock, &serverM_udp_addr);
